defmodule RulesTest do
  ...
  test "eat_ghost?/2 ghost does not get eaten \
        because no power pellet active" do
  ...
  test "eat_ghost?/2 ghost does not get eaten because no power \
        pellet is active, even if not touching ghost" do
  ...
  test "win?/3 win if all dots eaten and \
        touching a ghost with a power pellet active" do
