defmodule TakeANumberDeluxe.State do
  ...
  @spec min_and_max_numbers_valid?(number(), number()) :: boolean()
  ...
  @spec timeout_valid?(timeout()) :: boolean()
  defp timeout_valid?(timeout) do
    timeout == :infinity || (is_integer(timeout) && timeout >= 0)
  end

  @spec find_next_available_number(t()) ::
    {:ok, number()} | {:error, :all_possible_numbers_are_in_use}
